...
def ingest_player_info():
    try:
        ...
        for player in players:
            ...
            cursor.execute("""
                INSERT OR IGNORE INTO player_info (player_id, first_name, last_name, position, team_id, status, total_points, current_value, selected_by_percent, news, photo)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (player['id'], player['first_name'], player['second_name'], player['element_type'], player['team'], player['status'], player['total_points'], player['now_cost'], player['selected_by_percent'], player['news'], player['photo']))

            # Update the player's details
            cursor.execute("""
                UPDATE player_info
                SET first_name = ?, last_name = ?, position = ?, team_id = ?, status = ?, total_points = ?, current_value = ?, selected_by_percent = ?, news = ?, photo = ?
                WHERE player_id = ?
            """, (player['first_name'], player['second_name'], player['element_type'], player['team'], player['status'], player['total_points'], player['now_cost'], player['selected_by_percent'], player['news'], player['photo'], player['id']))
